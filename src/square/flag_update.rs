...
where FlagType<F1>: From<FlagType<F2>>, FlagType<F2>: FlElement<FlagItem=F2, Item=F2> + From<FlagType<F1>>{
    fn set_initial<'a> (&'a mut self, it: impl Iterator<Item = &'a Self>) {
        self.flags = it.fold
            (FlagType::max(),|acc, x| acc - <FlagType<F2>>::from(x.value));
...
impl <V: NormalInt, F2: Flag> FlagUpdate for FlagSquare<IntType<V>, FlagType<F2>>
    where FlagType<F2>: FlElement<FlagItem=F2, Item=F2> + From<IntType<V>>{
    fn set_initial<'a> (&'a mut self, it: impl Iterator<Item = &'a Self>) {
        self.flags = it.fold
        (FlagType::max(),|acc, x| acc - <FlagType<F2>>::from(x.value));
        self.count = FlagType::count_ones(&self.flags.get());
...
mod flag_update_tests {
    ...
    fn ft_ft_test() {
        ...
        let vals:Vec<FlagSquare<FlagType<u16>, FlagType<u16>>> = v.iter().map(|&e| FlagSquare::new(e, true)).collect();
    ...
    fn int_ft_test() {
        let v:Vec<u8> = vec![1, 3, 5];
        let vals:Vec<FlagSquare<IntType<u8>, FlagType<u16>>> = v.iter().map(|&e| FlagSquare::new(e, true)).collect();
